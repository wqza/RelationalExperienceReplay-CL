...
def store_mnist_masked_loaders(train_dataset: datasets, test_dataset: datasets,
                    setting: ContinualDataset) -> Tuple[DataLoader, DataLoader]:
    """
    Divides the MNIST dataset into tasks.
    :param train_dataset: train dataset
    :param test_dataset: test dataset
    :param setting: continual learning setting
    :return: train and test loaders
    """
    train_mask = np.logical_and(np.array(train_dataset.train_labels) >= setting.i,
        np.array(train_dataset.train_labels) < setting.i + setting.N_CLASSES_PER_TASK)
    test_mask = np.logical_and(np.array(test_dataset.test_labels) >= setting.i,
        np.array(test_dataset.test_labels) < setting.i + setting.N_CLASSES_PER_TASK)

    train_dataset.train_data = train_dataset.train_data[train_mask]
    test_dataset.test_data = test_dataset.test_data[test_mask]

    train_dataset.train_labels = np.array(train_dataset.train_labels)[train_mask]
    test_dataset.test_labels = np.array(test_dataset.test_labels)[test_mask]
...
def get_all_trained_loaders(train_dataset, test_dataset, setting):
    """
    Divides the dataset into tasks.
    :param train_dataset: train dataset
    :param test_dataset: test dataset
    ...
    :return: train and test loaders
    """
    train_mask = np.array(train_dataset.targets) < setting.i + setting.N_CLASSES_PER_TASK
